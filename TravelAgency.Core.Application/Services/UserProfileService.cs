...
namespace TravelAgency.Core.Application.Services
{
    public class UserProfileService : IUserProfileService
    {
        ...
        public UserProfileService(IAuthenticationRepository authenticationRepository , IHotelReservationRepository hotelReservationRepository , IHotelRepository hotelRepository)
        {
        ...
        public string ChangeProfile(string? token , UserToChangeProfileDto userProfileDto , IIdentityRepository identityRepository)
        {
            ...
            if (user.Email != userProfileDto.Email && identityRepository.FindUserByEmail(userProfileDto.Email) is not null)
                throw new BadRequest("Email already Exists.");

            if (user.UserName != userProfileDto.UserName && identityRepository.FindUserByEmail(userProfileDto.UserName) is not null)
                throw new BadRequest("User Name already Exists.");

            if (user.PhoneNumber is not null && user.PhoneNumber != userProfileDto.PhoneNumber && identityRepository.FindUserByPhoneNumber(userProfileDto.PhoneNumber!) is not null)
        ...
        public string UpdateReservation(string? token, ReservationToChangeDto reservationDto)
        {
            ...
            if (hotelReservation.UserId != authentication.UserId)
                throw new NotFound("reservation", $"Number {reservationDto.ReservationId}");
            ...
            List<HotelReservation?> roomReservations = _hotelReservationRepository.GetReservations(hotelReservation.HotelId, hotelReservation.RoomId)
                                                                                  .Where(r => r!.Id != reservationDto.ReservationId)
                                                                                  .ToList();
